...
*/
void
Rtti::Construct(const char* className, FourCC fcc, Creator creatorFunc, const Rtti* parentClass, int instSize)
{
...
/**
*/
Rtti::Rtti(const char* className, FourCC fcc, Creator creatorFunc, const Rtti* parentClass, int instSize)
{
...
/**
*/
Rtti::Rtti(const char* className, Creator creatorFunc, const Rtti* parentClass, int instSize)
{
...
Rtti::Create() const
{
    if (0 == this->creator)
    {
        _error("Rtti::Create(): Trying to create instance of abstract class '%s'!", this->name.AsCharPtr());
        return 0;
    }
    else
    {
...
{
    #if VORTEX_OBJECTS_USE_MEMORYPOOL    
