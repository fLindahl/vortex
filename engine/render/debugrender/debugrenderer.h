...
public:
	...
	///Draws a unit colored cone at position with rotation, radius and length
	...
	///Draws a unit colored circle at position with rotation and radius
	void DrawCircle(const Math::vector& position, const Math::quaternion& rotation, const float& radius, const Math::vec4& color, const RenderMode& renderModes = RenderMode::Normal, const float& lineWidth = 1.0f);
	///Draws a colored cirlce with transform
