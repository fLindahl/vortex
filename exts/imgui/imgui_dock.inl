...
struct DockContext
{
	...
	{
		tinyxml2::XMLDocument data;
		int result = data.LoadFile(filename.c_str());
		...
		const tinyxml2::XMLElement* docks = data.RootElement()->FirstChildElement();
		const tinyxml2::XMLElement* dockElement = docks->FirstChildElement();
		
		//Start by allocating memory for docks
		while (dockElement != nullptr)
		{
			Dock* new_dock = (Dock*)MemAlloc(sizeof(Dock));
			m_docks.push_back(IM_PLACEMENT_NEW(new_dock) Dock());
			dockElement = dockElement->NextSiblingElement();
		}

		dockElement = docks->FirstChildElement();
		// Insert data for each element
		while (dockElement != nullptr)
		{
			int idx = std::stoi(dockElement->FindAttribute("index")->Value());
			Dock& dock = *m_docks[idx];
			dock.last_frame = 0;
			dock.invalid_frames = 0;
			
			dock.label = ImStrdup(dockElement->FindAttribute("label")->Value());
			dock.id = ImHash(dock.label, 0);

			dock.pos.x = std::stof(dockElement->FindAttribute("x")->Value());
			dock.pos.y = std::stof(dockElement->FindAttribute("y")->Value());
			dock.size.x = std::stof(dockElement->FindAttribute("size_x")->Value());
			dock.size.y = std::stof(dockElement->FindAttribute("size_y")->Value());
			dock.status = (Status_)std::stoi(dockElement->FindAttribute("status")->Value());
			dock.active = std::stoi(dockElement->FindAttribute("active")->Value());
			dock.opened = std::stoi(dockElement->FindAttribute("opened")->Value());
			dock.prev_tab = getDockByIndex(std::stoi(dockElement->FindAttribute("prev")->Value()));
			dock.next_tab = getDockByIndex(std::stoi(dockElement->FindAttribute("next")->Value()));
			dock.children[0] = getDockByIndex(std::stoi(dockElement->FindAttribute("child0")->Value()));
			dock.children[1] = getDockByIndex(std::stoi(dockElement->FindAttribute("child1")->Value()));
			dock.parent = getDockByIndex(std::stoi(dockElement->FindAttribute("parent")->Value()));
