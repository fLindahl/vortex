...
static const float GRADIENT_BAR_WIDGET_HEIGHT = 20;
...
namespace ImGui
{
                                ...
                                float height, int id)
    {
        ...
        draw_list->AddRectFilled(ImVec2(bar_pos.x - 2, bar_pos.y - 2),
								 ...
								 IM_COL32(100, 100, 100, 0), 3.0f, ImGuiCorner_All);

		draw_list->AddImage((void*)id, ImVec2(bar_pos.x, bar_pos.y), ImVec2(bar_pos.x + maxWidth, barBottom), ImVec2(0.f,0.f), ImVec2(0.8f,0.05f));
    ...
    bool GradientButton(ImGradient* gradient, int id)
    {
        ...
        float maxWidth = ImMax(250.0f, ImGui::GetContentRegionAvailWidth() - 70.0f);
        bool clicked = ImGui::InvisibleButton("gradient_bar", ImVec2(maxWidth, GRADIENT_BAR_WIDGET_HEIGHT));
        
        DrawGradientBar(gradient, widget_pos, maxWidth, GRADIENT_BAR_WIDGET_HEIGHT, id);
    ...
    bool GradientEditor(ImGradient* gradient,
                        ImGradientMark* & draggingMark,
                        ImGradientMark* & selectedMark, int id)
    {
        if(!gradient) return false;
        ...
        bar_pos.x += 10;
        float maxWidth = ImGui::GetContentRegionAvailWidth() - 30;
        ...
        DrawGradientBar(gradient, bar_pos, maxWidth, GRADIENT_BAR_EDITOR_HEIGHT, id);
        ...
        if(selectedMark)
        {
			bool colorModified = ImGui::ColorPicker4("##colorMod", selectedMark->color);
